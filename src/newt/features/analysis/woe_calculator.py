    ...
    def fit(self, X: pd.Series, y: pd.Series) -> 'WOEEncoder':
        ...
        if self.is_numeric_ and X.nunique() > 20:
            ...
            try:
                ...
                _, self.bins_ = pd.qcut(X, q=self.buckets, duplicates='drop', retbins=True)
    ...
    def transform(self, X: pd.Series) -> pd.Series:
        ...
        if self.is_numeric_ and self.bins_ is not None:
            ...
            if self.is_numeric_: # Was numeric but low cardinality treated as cat
                X_binned = X.astype(str) # wait, fit converted low card num to str? Yes line 45.
                ...
                # So we should convert input to str if self.bins_ is None but self.is_numeric_ is True?
...
def calculate_woe_mapping(df: pd.DataFrame, target: str, feature: str, bins: int = 10, epsilon: float = 1e-8) -> Dict[Any, float]:
...
def apply_woe_transform(df: pd.DataFrame, feature: str, woe_map: Dict[Any, float], new_col_name: Optional[str] = None) -> pd.DataFrame:
    """
    Wrapper for applying WoE transform. 
    NOTE: This legacy function assumes the input matches the map keys (e.g. already binned if necessary),
    OR it does a direct map. It does NOT use the WOEEncoder's binning logic because it receives a raw dict.
