...
class DecisionTreeBinner(BaseBinner):
    ...
    """
    def __init__(self, n_bins: int = 5, force_monotonic: bool = False, min_samples_leaf: float = 0.05):
...
class ChiMergeBinner(BaseBinner):
    ...
    """
    def __init__(self, n_bins: int = 5, force_monotonic: bool = False, init_bins: int = 50, confidence_level: float = 0.95):
        super().__init__(n_bins=n_bins, force_monotonic=force_monotonic)
    ...
    def _fit_splits(self, X: pd.Series, y: Optional[pd.Series] = None) -> List[float]:
        ...
        try:
            _, init_splits = pd.qcut(X, q=self.init_bins, duplicates='drop', retbins=True)
        ...
        while len(splits) + 1 > self.n_bins:
            ...
            grouped = df.groupby('bin_code', observed=False)['target'].agg(['count', 'sum'])
            ...
            for i in range(len(indices) - 1):
                ...
                if total1 == 0 or total2 == 0:
                    ...
                    def safe_chi(obs, exp):
                        if exp < 1e-6: return 0.0
                        return (obs - exp)**2 / exp
                        
                    chi2 = (safe_chi(bad1, exp_bad1) + safe_chi(good1, exp_good1) +
                            safe_chi(bad2, exp_bad2) + safe_chi(good2, exp_good2))
