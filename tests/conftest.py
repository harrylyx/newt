...
def german_credit_data():
    ...
    data_path = os.path.join(base_path, "examples", "data", "statlog+german+credit+data", "german.data-numeric")
    ...
    # Target: 1 = Good, 2 = Bad. Remap to 0 = Good, 1 = Bad (standard binary classification)
    y = (y == 2).astype(int)
    ...
    return {
        ...
        "all": {
            "y_true": y,
            "y_prob": np.concatenate([y_prob_train, y_prob_test]) # Note: order mixed if not careful, but for metrics just distribution matters usually
        }
    }
