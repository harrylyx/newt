...
def test_correlation(analysis_data):
    df = analysis_data
    # 1. Matrix
    corr_matrix = calculate_correlation_matrix(df_matrix=df.drop(columns='target'))
...
def test_woe_calculator(analysis_data):
    ...
    # calculate_woe_mapping bins internally but returns a map keyed by Intervals (if cut/qcut used).
    ...
    woe_map_binned = calculate_woe_mapping(df, target='target', feature='x1_bin', bins=None)
    
    # Now transform
    df_transformed = apply_woe_transform(df, feature='x1_bin', woe_map=woe_map_binned)
    ...
    expected_woe = woe_map_binned[sample_val]
